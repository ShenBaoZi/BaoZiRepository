 ...
 * 
 */
public class GameFist extends GameWeapon {
	private int scope = 1;
	 ...
	 * @param drawer
	 */
	public GameFist(int x, int y, IDrawAction drawer) {
		...
		boomImage = Util.getInstance().createImage("wind.png");
		status = CREATE;
	}
	...
	public void tick(Graphics g) {
		...
		if (this.status == SET
				&& GameWorld.getInstance().getGameTime() - this.setTime >= Common.FIST_LEFT) {
			this.status = BOOM;
