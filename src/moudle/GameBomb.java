 ...
 * 
 */
public class GameBomb extends GameWeapon {
	private int scope = 3;
	 ...
	 * @param drawer
	 */
	public GameBomb(int x, int y, IDrawAction drawer) {
		...
		boomImage = Util.getInstance().createImage("fire.png");
		status = CREATE;
	}
	...
	public void tick(Graphics g) {
		...
		if (this.status == SET
				&& GameWorld.getInstance().getGameTime() - this.setTime >= Common.BOOM_LEFT) {
			this.status = BOOM;
